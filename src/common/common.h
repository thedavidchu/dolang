 ...
 * ------
 * cond_ : int
 *     condition that is evaluated; a 'true' result will cause the 'err_' to
 * return. err_ : int (no side effects) error value to be returned if the
 * condition is met. Notes
 * -----
 * * cond_ is evaluated only once; err_ may be evaluated many times.
 */
#define RETURN_IF_ERROR(cond_, err_)                                           \
    do {                                                                       \
        ...
        if ((_err = (int)(cond_))) {                                           \
            ...
            _err = print_stderr("[ERROR] %s:%d: err_ = \"%s\" = %d: \"%s\"\n"  \
                                "\tcond_ = \"%s\" = %d\n",                     \
                                __FILE__, __LINE__, #err_, (int)(err_),        \
                                _safe_strerror((int)(err_)), #cond_, _err);    \
...
#define REQUIRE_NO_ERROR(cond_, msg_)                                          \
    do {                                                                       \
        int _err = 0; /* err_ cannot be '_err'. */                             \
        if ((_err = (int)(cond_))) {                                           \
            const int old_errno = errno;                                       \
