...
int tbl_ctor(tbl *const restrict me, size_t cap,
    ...
    RETURN_IF_ERROR(
        (err = mem_malloc((void **)&me->table, cap, sizeof *me->table)), err);
...
int tbl_insert(tbl *const restrict me, void *const key, void *const value,
    ...
    RETURN_IF_ERROR(
        err = tbl_gettableidx(me, key, /*return_tombstone=*/true, &table_idx),
        err);
    ...
    if (item_idx == INVALID || item_idx == TOMBSTONE) {
        ...
        ++me->len;
        assert(me->items.len == new_item_idx + 1 &&
               "unexpected length of items");
        return 0;
    } else {
...
void *tbl_search(tbl *const restrict me, const void *const key) {
    ...
    if ((err = tbl_gettableidx(me, key, /*return_tombstone=*/false,
                               &table_idx))) {
        return NULL;
    }
...
int tbl_remove(tbl *const restrict me, const void *const key,
    ...
    RETURN_IF_ERROR(err =
                        tbl_gettableidx(me, key,
                                        /*return_tombstone=*/false, &table_idx),
                    err);
    if (table_idx == INVALID) {
        return 0;
    }
