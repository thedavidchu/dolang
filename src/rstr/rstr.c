...
int rstr_dtor(rstr *const restrict me, int (*str_dtor)(char *const)) {
    ...
    REQUIRE_NO_ERROR(str_dtor(me->str), "failed to free string");
    me->str = NULL;
    me->len = 0;
    return 0;
}
...
int rstr_cmp(const rstr *const me, const rstr *const other) {
    ...
    return strncmp(me, other, me->len);
}

int rstr_debug(const rstr *const restrict me) {
    int r = 0;

    RETURN_IF_ERROR(me, ERROR_NULLPTR);
    RETURN_IF_ERROR(me->str, ERROR_NULLPTR);
    RETURN_IF_ERROR(me->len > (size_t)INT_MAX, ERROR_OUTOFBOUNDS);

    r = printf("(len=%d)'%.*s'", (int)me->len, (int)me->len, me->str);
    return (r < 0) ? ERROR : 0;
