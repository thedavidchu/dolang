...
- [ ] Make this more user-friendly
    - Pass token start position + length (we can work out the line number/column here)
    -
...
def print_analyzer_error(text: str, token: Token, error_msg: str) -> None:
    print(
        "--------------------------------------------------------------------------------"
    )
    print(
        f"Analysis error on line "
        f"{token.line_number}, column {token.column_number}"
    )
    print(f"Error Message: {error_msg}")
    print("```")
