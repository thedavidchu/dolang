...
class LolIRFunctionCallExpression:
    def __init__(
        self,
        function: "LolAnalysisFunction",
        arguments: List["LolAnalysisVariable"],
    ):
    ...
    def __str__(self):
        return (
            f"{self.function.name}{tuple(arg.name for arg in self.arguments)}"
        )
...
class LolIROperatorExpression:
    def __init__(self, op: str, operands: List["LolAnalysisVariable"]):
        ...
        assert all(
            isinstance(operand, LolAnalysisVariable) for operand in operands
        )
        self.op = op
...
class LolIRDefinitionStatement:
    def __init__(
        self, name: str, type: "LolAnalysisDataType", value: LolIRExpression
    ):
...
LolAnalysisSymbol = Union[
    LolAnalysisDataType, "LolAnalysisFunction", "LolAnalysisVariable"
]
...
class LolAnalysisVariable:
    def __init__(
        self,
        name: str,
        ast_definition_node: Optional[
            Union[LolParserVariableDefinition, LolParserParameterDefinition]
        ],
        *,
        ...
        assert (
            isinstance(
                ast_definition_node,
                (LolParserVariableDefinition, LolParserParameterDefinition),
            )
            or ast_definition_node is None
        )
        self.name = name
...
class LolAnalysisFunction:
    def __init__(
        ...
        *,
        ...
        self.parameter_types: Optional[List[LolAnalysisDataType]] = (
            parameter_types
        )
    ...
    def complete_prototype(
        ...
        self.return_types = get_type(
            self.ast_definition_node.return_type, module_symbol_table
        )
        self.parameter_types = [
            get_type(t.type, module_symbol_table)
    ...
    def _get_symbol(
        self, module_symbol_table: Dict[str, LolAnalysisSymbol], name: str
    ) -> LolAnalysisSymbol:
    ...
    def _get_operator_return_type(
        ...
        hacky_ret_type = self._get_symbol(
            module_symbol_table, first_operand.name
        ).type
        ...
        *,
        ...
        if isinstance(x, LolParserOperatorExpression):
            ...
            operands: List["LolAnalysisVariable"] = [
                self._get_symbol(
                    module_symbol_table,
                    self._parse_expression_recursively(
                        y, module_symbol_table, body_block=body_block
                    ),
                )
                for y in x.operands
            ]
            ...
            ret_type = self._get_operator_return_type(
                module_symbol_table, op_name, operands
            )
            ret_value = LolIROperatorExpression(op_name, operands)
            stmt = LolIRDefinitionStatement(ret, ret_type, ret_value)
            body_block.append(stmt)
            self.symbol_table[ret] = LolAnalysisVariable(
                ret, None, type=ret_type
            )
            return ret
            ...
            func: LolAnalysisFunction = self._get_symbol(
                module_symbol_table, func_name
            )
            ...
            ret = self._parse_expression_recursively(
                x.value, module_symbol_table, body_block=body_block
            )
            stmt = LolIRReturnStatement(
                self._get_symbol(module_symbol_table, ret)
            )
            body_block.append(stmt)
        elif isinstance(x, LolParserIfStatement):
            if_cond_name = self._parse_expression_recursively(
                x.if_condition, module_symbol_table, body_block=body_block
            )
            if_cond = self._get_symbol(module_symbol_table, if_cond_name)
            if_block = []
            for y in x.if_block:
                self._parse_statement(
                    module_symbol_table, y, body_block=if_block
                )
            else_block = []
            for y in x.else_block:
                self._parse_statement(
                    module_symbol_table, y, body_block=else_block
                )
    ...
    def _parse_statement(
        ...
        *,
        ...
        if isinstance(x, LolParserVariableDefinition):
            ...
            value = self._parse_expression_recursively(
                x.value, module_symbol_table, body_block=body_block
            )
            self.symbol_table[name] = LolAnalysisVariable.init_local_variable(
                name, x, module_symbol_table
            )
            stmt = LolIRDefinitionStatement(
            ...
            _unused_return_variable = self._parse_expression_recursively(
                x, module_symbol_table, body_block=body_block
            )
    ...
    def complete_body(self, module_symbol_table: Dict[str, LolAnalysisSymbol]):
        ...
        for statement in self.ast_definition_node.body:
            self._parse_statement(
                module_symbol_table, statement, body_block=self.body
            )
...
class LolAnalysisModule:
    def __init__(
        self, name: str, caller_module: Optional["LolAnalysisModule"] = None
    ):
        self.name = name
    ...
    def add_function_prototype(
        self, ast_definition: LolParserFunctionDefinition
    ):
        name = ast_definition.get_name_as_str()
        func: LolAnalysisFunction = self.module_symbol_table[name]
        func.complete_prototype(self.module_symbol_table)

    def add_variable_prototype(
        self, ast_definition: LolParserVariableDefinition
    ):
        name = ast_definition.get_name_as_str()
    ...
    def get_module_prototypes(
        self, ast_nodes: List[LolParserModuleLevelStatement]
    ):
...
def analyze(
    asts: List[LolParserModuleLevelStatement], raw_text: str
) -> LolAnalysisModule:
