...
#define TEST_INT_EQ(output, oracle, err)                                       \
    do {                                                                       \
        ...
        int _oracle = (oracle);                                                \
        if (_output == _oracle) {                                              \
            fprintf(CHANNEL, /*GREEN*/ "\033[32m" #output " == " #oracle       \
                                       ": OK\033[0m\n" /*END GREEN*/);         \
        } else {                                                               \
            fprintf(CHANNEL, /*RED*/                                           \
                    "\033[31m" #output "(=%d) != " #oracle "(=%d)"             \
...
int test_mem(void) {
    ...
    TEST_INT_EQ(mem_realloc(NULL, 0, 1), -1,
                err); /* Pass in NULL ptr => error*/
    ...
    TEST_PTR_EQ(p, NULL, err);
    /* errno still set! */
    TEST_INT_EQ(mem_realloc(NULL, 0, 1), ENOMEM,
                err); /* Pass in NULL ptr => error*/
...
int test_rstr(void) {
    ...
    REQUIRE_NO_ERROR(rstr_ctor(&full, str, strlen(str)),
