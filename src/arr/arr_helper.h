...
static inline int arr_copyitem_nocheck(const arr *const me,
...
static inline int arr_copyitem_nocheck(const arr *const me, const size_t idx,
                                       void *const item) {
    int err = 0;

    if ((err = mem_memcpy(/*src=*/item, /*dst=*/ARR_GETITEM(me, idx), /*num=*/1,
                          /*size=*/me->size))) {
        return err;
    }
    return 0;
